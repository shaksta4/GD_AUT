...
BackBuffer::DrawSprite(Sprite& sprite)
{
	...
	anchor.x = sprite.GetWidth()/2; 
	anchor.y = sprite.GetHeight()/2;

	SDL_RenderCopyEx(m_pRenderer, sprite.GetTexture()->GetTexture(), 0, &dest, sprite.GetAngle(), &anchor, SDL_FLIP_NONE);
}
